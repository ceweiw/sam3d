...
def get_matching_indices(pcd0, pcd1, search_voxel_size, K=None):
    #这个函数本身是遍历参数中的pcd1为pcd0中的每个点找匹配点
    #第一次调用的参数是 get_matching_indices(pcd1, pcd0, 1.5 * voxel_size, 1)  即遍历pcd0为pcd1找最近点匹配点 那么match_inds的i是pcd1的索引
    match_inds\
        = []
    scene_coord = torch.tensor(np.asarray(pcd0.points)).cuda().contiguous().float() #pcd0.points存储的是点云的坐标 sam3d里的代码有看到
    new_offset = torch.tensor(scene_coord.shape[0]).cuda().float()  #pcd0点云的点数
    gen_coord = torch.tensor(np.asarray(pcd1.points)).cuda().contiguous().float()   #
    offset = torch.tensor(gen_coord.shape[0]).cuda().float()        #pcd1点云的点数
    #这里的offset打印出来是tensor(2790., device='cuda:0')这样一个标量
    indices, dis = pointops.knn_query(1, gen_coord, offset, scene_coord, new_offset)
    #选取第一个最近邻的索引和距离
    indices = indices.cpu().numpy()[:,0]
    dis = dis.cpu().numpy()[:,0]

    for i in range(scene_coord.shape[0]):  #遍历pcd0中的点
        if dis[i] < search_voxel_size:
            match_inds.append([i,indices[i]])
    #这个函数是为pcd0计算pcd1最近点的匹配情况
    return match_inds
