...
class KNNQuery(Function):
    ...
    def forward(ctx, nsample, xyz, offset, new_xyz=None, new_offset=None):
        ...
        m = new_xyz.shape[0]
        idx = torch.cuda.IntTensor(m, nsample).zero_() #存储查询点的最近邻点索引
        dist2 = torch.cuda.FloatTensor(m, nsample).zero_()  #最近邻点的距离
        #nsample每个查询点所需的邻居数
        knn_query_cuda(m, nsample, xyz, new_xyz, offset.int(), new_offset.int(), idx, dist2)  #这里是强转成了int类型
