...
def read_plymesh(filepath): #这里是在加载scene0059_00_vh_clean_2.ply
    """Read ply file and return it as numpy array. Returns None if emtpy."""
    with open(filepath, 'rb') as f:       #高质量的重建表面网格数据（经过清理和简化的版本  按这里的代码里面是有点和面的数据的  而且这个文件里面是一整个场景ply文件
        plydata = plyfile.PlyData.read(f)  #但其实最后只取了里面点的颜色和坐标信息 （后面还是要用面的信息
    if plydata.elements:
...
def vertex_normal(vertex, face):
    ...
    for i in range(face.shape[0]):      #计算顶点法线
        nv[face[i]] += nf[i]
...
def handle_process(scene_path, output_path, labels_pd, train_scenes, val_scenes, parse_normals=True):
    ...
    mesh_path = os.path.join(scene_path, f'{scene_id}{CLOUD_FILE_PFIX}.ply')  #scene0059_00_vh_clean_2.ply
    segments_file = os.path.join(scene_path, f'{scene_id}{CLOUD_FILE_PFIX}{SEGMENTS_FILE_PFIX}')  #scene0059_00_vh_clean_2.0.010000.segs.json
    aggregations_file = os.path.join(scene_path, f'{scene_id}{AGGREGATIONS_FILE_PFIX}')  #scene0059_00.aggregation.json
    info_file = os.path.join(scene_path, f'{scene_id}.txt')   #scene0059_00.txt
    ...
    # Load segments file
    if split_name != "test":
        with open(segments_file) as f:    #点云的分组信息 是一个数组 也是针对一个场景的
            segments = json.load(f)
            seg_indices = np.array(segments['segIndices'])

        # Load Aggregations file
        with open(aggregations_file) as f:   #实例标注 大概就是哪些点属于一个实例 标签是什么
